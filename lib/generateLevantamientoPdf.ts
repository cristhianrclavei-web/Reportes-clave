...
export async function generateLevantamientoPdf(
  ...
  // Las fotos del celular pesan varios MB cada una; sin reducirlas, un
  // levantamiento con 10+ fotos produce un PDF de decenas de MB, y Vercel
  // rechaza respuestas de más de 4.5 MB. A ~1000 px por lado se ven bien en
  // media hoja y el PDF queda en ~1 MB. `rotate()` respeta la orientación
  // EXIF, para que las fotos verticales no salgan de lado.
  async function comprimirFoto(bytes: Uint8Array): Promise<Uint8Array> {
    try {
