...
## Importante: los patches de SQL

Subir el código NO actualiza la base de datos. Cada vez que un cambio agrega
columnas, funciones o políticas, hay un archivo en `supabase/` que hay que
ejecutar a mano en el **SQL Editor** de Supabase.

Si la app muestra "column ... does not exist", una lista vacía sin
explicación, o funciones nuevas que no responden, lo primero que hay que
revisar es si falta correr un patch.

El detalle de qué hace cada uno y en qué orden va está en
[CHANGELOG.md](CHANGELOG.md).

Para ver qué está pasando del lado del servidor (sesión, rol, conteo de
reportes, errores) existe la ruta `/diagnostico`.
