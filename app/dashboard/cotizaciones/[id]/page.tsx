...
export default async function CotizacionDetallePage(props: { params: Promise<{ id: string }> }) {
  ...
  const { data: myProfile } = await supabase.from('profiles').select('full_name, can_approve_review').eq('id', user.id).single();
  ...
  return (
    <CotizacionDetalle
      ...
      puedeAprobar={Boolean(myProfile?.can_approve_review)}
    />
  );
}
